...
const pathToKey: string = path.join(__dirname, '../server/cryptography/', 'id_rsa_priv.pem');
...
function validPassword(password: string, hash: string, salt: string) {
    let hashVerify: string = crypto.pbkdf2Sync(password, salt, 10000, 64, 'sha512').toString('hex');
...
function issueJWT(user: User) {
    ...
    const signedToken: string = jwt.sign(payload, PRIVATE_KEY, {expiresIn: expiresIn, algorithm: 'RS256'});
